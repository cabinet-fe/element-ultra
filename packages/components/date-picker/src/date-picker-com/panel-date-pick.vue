...
const defaultTimeD = computed(() => {
  return dayjs(defaultTime).locale(lang)
})
...
const selectionMode = computed(() => {
  const { type } = props
  const types = ['week', 'month', 'year', 'dates'] as const
  if (types.includes(type as any)) return type as typeof types[number]
  return 'date'
})
...
const formatToString = (value: Dayjs | Dayjs[]) => {
  ...
  if (selectionMode.value === 'week') {
    return (value as Dayjs).format('第 ww 周')
  }
...
const handleKeydownTable = (event: KeyboardEvent) => {
  const code = event.code as EVENT_CODE
  const validCode = [
    EVENT_CODE.up,
    EVENT_CODE.down,
    EVENT_CODE.left,
...
const handleKeyControl = (code: string) => {
  ...
  const home = EVENT_CODE.home
  const end = EVENT_CODE.end
  const pageUp = EVENT_CODE.pageUp
  const pageDown = EVENT_CODE.pageDown
